...
public class ItemsResourceIT {
    ...
    public static Items createEntity(EntityManager em) {
        ...
        // Add required entity
        Designs designs;
        if (TestUtil.findAll(em, Designs.class).isEmpty()) {
            designs = DesignsResourceIT.createEntity(em);
            em.persist(designs);
            em.flush();
        } else {
            designs = TestUtil.findAll(em, Designs.class).get(0);
        }
        items.setRelatedDesign(designs);
        // Add required entity
        Products products;
        ...
        if (TestUtil.findAll(em, UnitOfMeasurements.class).isEmpty()) {
            unitOfMeasurements = UnitOfMeasurementsResourceIT.createEntity(em);
            em.persist(unitOfMeasurements);
            em.flush();
        } else {
            unitOfMeasurements = TestUtil.findAll(em, UnitOfMeasurements.class).get(0);
        }
        items.setUnitOfMeasurements(unitOfMeasurements);
        // Add required entity
        Currency currency;
    ...
    public static Items createUpdatedEntity(EntityManager em) {
        ...
        Designs designs;
        if (TestUtil.findAll(em, Designs.class).isEmpty()) {
            designs = DesignsResourceIT.createUpdatedEntity(em);
            em.persist(designs);
            em.flush();
        } else {
            designs = TestUtil.findAll(em, Designs.class).get(0);
        ...
        UnitOfMeasurements unitOfMeasurements;
        if (TestUtil.findAll(em, UnitOfMeasurements.class).isEmpty()) {
            unitOfMeasurements = UnitOfMeasurementsResourceIT.createUpdatedEntity(em);
            em.persist(unitOfMeasurements);
            em.flush();
        } else {
            unitOfMeasurements = TestUtil.findAll(em, UnitOfMeasurements.class).get(0);
    ...
    public void getAllItemsByRelatedDesignIsEqualToSomething() throws Exception {
        ...
        Designs relatedDesign = items.getRelatedDesign();
        itemsRepository.saveAndFlush(items);
    ...
    @Test
    @Transactional
    public void getAllItemsByUnitOfMeasurementsIsEqualToSomething() throws Exception {
        // Get already existing entity
        UnitOfMeasurements unitOfMeasurements = items.getUnitOfMeasurements();
        itemsRepository.saveAndFlush(items);
        Long unitOfMeasurementsId = unitOfMeasurements.getId();

        // Get all the itemsList where unitOfMeasurements equals to unitOfMeasurementsId
        defaultItemsShouldBeFound("unitOfMeasurementsId.equals=" + unitOfMeasurementsId);

        // Get all the itemsList where unitOfMeasurements equals to unitOfMeasurementsId + 1
        defaultItemsShouldNotBeFound("unitOfMeasurementsId.equals=" + (unitOfMeasurementsId + 1));
    }
