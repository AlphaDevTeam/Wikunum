...
public class ItemsResourceIT {
    ...
    public static Items createEntity(EntityManager em) {
        ...
        // Add required entity
        UOM uOM;
        if (TestUtil.findAll(em, UOM.class).isEmpty()) {
            uOM = UOMResourceIT.createEntity(em);
            em.persist(uOM);
            em.flush();
        } else {
            uOM = TestUtil.findAll(em, UOM.class).get(0);
        }
        items.setUOM(uOM);
        // Add required entity
        Currency currency;
    ...
    public static Items createUpdatedEntity(EntityManager em) {
        ...
        UOM uOM;
        if (TestUtil.findAll(em, UOM.class).isEmpty()) {
            uOM = UOMResourceIT.createUpdatedEntity(em);
            em.persist(uOM);
            em.flush();
        } else {
            uOM = TestUtil.findAll(em, UOM.class).get(0);
    ...
    @Test
    @Transactional
    public void getAllItemsByUOMIsEqualToSomething() throws Exception {
        // Get already existing entity
        UOM uOM = items.getUOM();
        itemsRepository.saveAndFlush(items);
        Long uOMId = uOM.getId();

        // Get all the itemsList where uOM equals to uOMId
        defaultItemsShouldBeFound("uOMId.equals=" + uOMId);

        // Get all the itemsList where uOM equals to uOMId + 1
        defaultItemsShouldNotBeFound("uOMId.equals=" + (uOMId + 1));
    }
