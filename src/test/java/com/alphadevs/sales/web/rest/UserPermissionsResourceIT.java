...
public class UserPermissionsResourceIT {
    ...
    @Test
    @Transactional
    public void getAllUserPermissionsByUserPermissionsIsEqualToSomething() throws Exception {
        // Initialize the database
        userPermissionsRepository.saveAndFlush(userPermissions);
        ExUser userPermissions = ExUserResourceIT.createEntity(em);
        em.persist(userPermissions);
        em.flush();
        userPermissions.addUserPermissions(userPermissions);
        userPermissionsRepository.saveAndFlush(userPermissions);
        Long userPermissionsId = userPermissions.getId();

        // Get all the userPermissionsList where userPermissions equals to userPermissionsId
        defaultUserPermissionsShouldBeFound("userPermissionsId.equals=" + userPermissionsId);

        // Get all the userPermissionsList where userPermissions equals to userPermissionsId + 1
        defaultUserPermissionsShouldNotBeFound("userPermissionsId.equals=" + (userPermissionsId + 1));
    }


    @Test
    @Transactional
    public void getAllUserPermissionsByUserGroupsIsEqualToSomething() throws Exception {
        // Initialize the database
        userPermissionsRepository.saveAndFlush(userPermissions);
        UserGroups userGroups = UserGroupsResourceIT.createEntity(em);
        em.persist(userGroups);
        em.flush();
        userPermissions.addUserGroups(userGroups);
        userPermissionsRepository.saveAndFlush(userPermissions);
        Long userGroupsId = userGroups.getId();

        // Get all the userPermissionsList where userGroups equals to userGroupsId
        defaultUserPermissionsShouldBeFound("userGroupsId.equals=" + userGroupsId);

        // Get all the userPermissionsList where userGroups equals to userGroupsId + 1
        defaultUserPermissionsShouldNotBeFound("userGroupsId.equals=" + (userGroupsId + 1));
    }
