...
public class UserPermissionsResourceIT {
    ...
    public static UserPermissions createEntity(EntityManager em) {
        ...
        userPermissions.setUser(exUser);
        // Add required entity
        MenuItems menuItems;
    ...
    @Test
    @Transactional
    public void getAllUserPermissionsByUserIsEqualToSomething() throws Exception {
        // Get already existing entity
        ExUser user = userPermissions.getUser();
        userPermissionsRepository.saveAndFlush(userPermissions);
        Long userId = user.getId();

        // Get all the userPermissionsList where user equals to userId
        defaultUserPermissionsShouldBeFound("userId.equals=" + userId);

        // Get all the userPermissionsList where user equals to userId + 1
        defaultUserPermissionsShouldNotBeFound("userId.equals=" + (userId + 1));
    }
    ...
    public void getAllUserPermissionsByUserGroupIsEqualToSomething() throws Exception {
        ...
        em.flush();
        userPermissions.setUserGroup(userGroup);
        userPermissionsRepository.saveAndFlush(userPermissions);
